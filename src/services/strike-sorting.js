...
const ticketStrikeMessage = async (message) => {
  ...
  const serverId = message.guild.id;
  const tickets = isolateTickets();
  const { day, month } = currentDate();
  try {
    let reply = `Strikes for ${day} ${month}: \n\n`;
    let awayReply = `The following Members are excused due to being ${roleName.Value}: \n\n`;

    // Reset Monthly Stikes
    if (day === '1') {
      resetMonthlyStrikes();
    }

    // Continue Strike Message
    for (i in mentions) {
      let user = i;
      if (
        i.roles.cache.some((role) => role.name === awayRole.Value) === true
      ) {
        awayReply = awayReply + `${user.username}\n`;
        continue;
      }
  ...
  // const awayRole = getAwayRole(message, serverId)

  return 'Hello';
};

const resetMonthlyStrikes = () => {};
